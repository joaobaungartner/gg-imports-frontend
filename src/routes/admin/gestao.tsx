...
import {
  adjustStock, createCategory, createCoupon, downloadOrdersCsv, getAuditLog,
  getLowStock, getSalesReport, getStockMovements, listAdminClients,
  listAllCategories, listCoupons, updateAdminClientStatus, updateCategory, updateCoupon,
...
function AdminManagementPage() {
  ...
  async function load() {
    try {
      const [r, c, cp, l, m, cl, a] = await Promise.all([
        getSalesReport(), listAllCategories(), listCoupons(), getLowStock(),
        getStockMovements(), listAdminClients(), getAuditLog(),
      ]);
      setReport(r); setCategories(c); setCoupons(cp); setLow(l); setMovements(m); setClients(cl); setLogs(a);
  ...
  return <div className="section-canvas min-h-screen"><div className="container-page py-10 space-y-8">
    ...
    <section className="grid gap-6 xl:grid-cols-2">
      <div className="surface-card p-6"><h2 className="editorial-title text-2xl">Categorias</h2>
        <form className="my-4 flex gap-2" onSubmit={async e => { e.preventDefault(); await createCategory({ nome: categoryName }); setCategoryName(""); await load(); }}><input className="input-field flex-1" value={categoryName} onChange={e=>setCategoryName(e.target.value)} placeholder="Nova categoria" required/><button className="btn-primary">Adicionar</button></form>
        <div className="space-y-2">{categories.map(c => <div className="flex items-center justify-between rounded-xl border p-3" key={c.id}><div><b>{c.nome}</b><p className="text-xs text-[var(--color-muted)]">{c.ativo ? 'Ativa' : 'Inativa'}</p></div><button className="btn-ghost" onClick={async()=>{await updateCategory(c.id,{ativo:!c.ativo}); await load();}}>{c.ativo?'Desativar':'Ativar'}</button></div>)}</div>
      </div>
      <div className="surface-card p-6"><h2 className="editorial-title text-2xl">Cupons</h2>
        <form className="my-4 grid grid-cols-2 gap-2" onSubmit={async e=>{e.preventDefault(); await createCoupon(coupon); setCoupon({codigo:'',desconto:10,validade:''}); await load();}}><input className="input-field" value={coupon.codigo} onChange={e=>setCoupon({...coupon,codigo:e.target.value.toUpperCase()})} placeholder="Código" required/><input className="input-field" type="number" min="1" max="100" value={coupon.desconto} onChange={e=>setCoupon({...coupon,desconto:Number(e.target.value)})}/><input className="input-field col-span-2" type="date" value={coupon.validade} onChange={e=>setCoupon({...coupon,validade:e.target.value})} required/><button className="btn-primary col-span-2">Criar cupom</button></form>
        <div className="space-y-2">{coupons.map(c=><div className="flex items-center justify-between rounded-xl border p-3" key={c.id}><div><b>{c.codigo}</b><p className="text-xs">{c.desconto}% · {new Date(c.validade+'T00:00:00').toLocaleDateString('pt-BR')}</p></div><button className="btn-ghost" onClick={async()=>{await updateCoupon(c.id,{ativo:!c.ativo});await load();}}>{c.ativo?'Desativar':'Ativar'}</button></div>)}</div>
      </div>
    </section>
    <section className="surface-card p-6"><h2 className="editorial-title text-2xl">Estoque baixo</h2><div className="mt-4 overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr><th>Produto</th><th>Tamanho</th><th>Estoque</th><th>Ajuste</th></tr></thead><tbody>{low.map(p=><tr className="border-t" key={p.id}><td className="py-3">{p.name}<br/><small>{p.sku}</small></td><td>{p.size}</td><td>{p.stock}</td><td><button className="btn-secondary" onClick={async()=>{const raw=prompt('Quantidade a adicionar (use negativo para saída):','1'); const reason=prompt('Motivo do ajuste:','Reposição manual'); if(raw&&reason){await adjustStock(p.id,Number(raw),reason);await load();}}}>Ajustar</button></td></tr>)}</tbody></table></div></section>
    <section className="grid gap-6 xl:grid-cols-2">
      <div className="surface-card p-6"><h2 className="editorial-title text-2xl">Clientes</h2><div className="mt-4 max-h-96 space-y-2 overflow-auto">{clients.map(c=><div className="flex justify-between gap-3 rounded-xl border p-3" key={c.id}><div><b>{c.name}</b><p className="text-xs">{c.email} · {c.order_count} pedidos · {money(c.total_spent)}</p></div><button className="btn-ghost" onClick={async()=>{await updateAdminClientStatus(c.id,!c.active);await load();}}>{c.active?'Bloquear':'Ativar'}</button></div>)}</div></div>
